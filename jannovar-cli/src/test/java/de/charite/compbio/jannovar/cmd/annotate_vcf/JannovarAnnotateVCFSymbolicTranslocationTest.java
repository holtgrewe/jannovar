...
/**
 * Run the annotate command for a symbolic inversion allele
 */
public class JannovarAnnotateVCFSymbolicTranslocationTest {
	...
	@Test
	public void testOnSymbolicDellyTranslocationAllele() throws IOException, URISyntaxException, CommandLineParsingException,
			HelpRequestedException, JannovarException {
		final File outFolder = tmpFolder.newFolder();
		final String inputFilePath = this.getClass().getResource("/symbolic_tra.vcf").toURI().getPath();
		...
		File f = new File(outFolder.getAbsolutePath() + File.separator + "symbolic_tra.jv.vcf");
		Assert.assertTrue(f.exists());

		final File expectedFile = new File(this.getClass().getResource("/symbolic_tra.jv.vcf").toURI().getPath());
