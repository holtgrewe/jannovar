...
/**
 * Run the annotate command for a symbolic deletion allele
 */
public class JannovarAnnotateVCFSymbolicDeletionTest {
	...
	public void testOnSymbolicDeletionAllele() throws IOException, URISyntaxException, CommandLineParsingException,
		...
		final String inputFilePath = this.getClass().getResource("/symbolic_del.vcf").toURI().getPath();
		...
		File f = new File(outFolder.getAbsolutePath() + File.separator + "symbolic_del.jv.vcf");
		Assert.assertTrue(f.exists());

		final File expectedFile = new File(this.getClass().getResource("/symbolic_del.jv.vcf").toURI().getPath());
