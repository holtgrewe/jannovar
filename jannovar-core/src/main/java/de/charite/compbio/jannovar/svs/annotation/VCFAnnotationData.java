...
// TODO(holtgrewe): merge back into annotation package
...
class VCFAnnotationData {
	...
	public void setTranscriptAndVariant(TranscriptModel tm, StructuralVariant variant) {
		if (tm == null)
			return;
		...
		if (effects.contains(VariantEffect.INTERGENIC_VARIANT) || effects.contains(VariantEffect.UPSTREAM_GENE_VARIANT)
			...
			if (variant.getAffectedIntervalOuter().isLeftOf(tm.getTXRegion().getGenomeBeginPos()))
				this.distance = tm.getTXRegion().getGenomeBeginPos()
						.differenceTo(variant.getAffectedIntervalOuter().getGenomeEndPos());
			else
				this.distance = variant.getAffectedIntervalOuter().getGenomeBeginPos()
